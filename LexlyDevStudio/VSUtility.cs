...
namespace LexlyDevstudio
{
    ...
    public static class VSUtility
    {
        ...
        public static string GetProjectLanguageFromItem(EnvDTE.ProjectItem item)
        {
            string guid = null;
            if (null == item.FileCodeModel)
            {
                if (null == item.ContainingProject)
                    return null;
                if (null == item.ContainingProject.CodeModel)
                    return null;
                guid = item.ContainingProject.CodeModel.Language;
            }
            else
                guid = item.FileCodeModel?.Language;
            switch (guid)
            {
